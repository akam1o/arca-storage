...
def allocate_vlan_ifname(
    svm_name: str, vlan_id: int, *, max_attempts: int = 256
) -> str:
    """
    ...
    for attempt in range(max_attempts):
        ...
        if not _ifname_exists_in_root(candidate):
            return candidate
    raise RuntimeError(
        "Failed to allocate a unique VLAN interface name (too many collisions)"
    )
...
def create_namespace(name: str) -> None:
    ...
    result = _run(["ip", "netns", "list"], capture_output=True, text=True, check=False)
    ...
    # Create namespace
    result = _run(
        ["ip", "netns", "add", name], capture_output=True, text=True, check=False
    )
...
def attach_vlan(
    ...
    result = _run(
        ["ip", "link", "show", vlan_if], capture_output=True, text=True, check=False
    )
    ...
    if result.returncode == 0:
        ...
        if result.returncode == 0:
            ...
            # Move to namespace
            _run(["ip", "link", "set", vlan_if, "netns", namespace], check=True)
    else:
...
def _configure_ip(
    namespace: str, interface: str, ip_cidr: str, gateway: Optional[str], mtu: int
) -> None:
...
def delete_namespace(name: str) -> None:
    ...
    result = _run(
        ["ip", "netns", "del", name], capture_output=True, text=True, check=False
    )
