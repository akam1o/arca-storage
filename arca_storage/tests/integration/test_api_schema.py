...
def test_openapi_schema_uses_phase_status_values_and_typed_data_envelopes():
    ...
    assert schemas["VolumeQoSResponse"]["properties"]["data"] == {"$ref": "#/components/schemas/VolumeQoSData"}
    assert schemas["VolumeQoSData"]["properties"]["qos"] == {"$ref": "#/components/schemas/VolumeQoS"}
    assert schemas["DeletedResponse"]["properties"]["data"] == {"$ref": "#/components/schemas/DeletedData"}
    assert schemas["DirectoryResponse"]["properties"]["data"] == {"$ref": "#/components/schemas/DirectoryData"}
    assert schemas["DirectoryData"]["properties"]["directory"] == {"$ref": "#/components/schemas/Directory"}
    assert schemas["QuotaResponse"]["properties"]["data"] == {"$ref": "#/components/schemas/QuotaData"}
    assert schemas["QoSRemovalResponse"]["properties"]["data"] == {"$ref": "#/components/schemas/QoSRemovalData"}
    assert schemas["SVMCapacityResponse"]["properties"]["data"] == {"$ref": "#/components/schemas/SVMCapacityData"}
    assert schemas["SVMCapacityData"]["properties"]["capacity"] == {"$ref": "#/components/schemas/SVMCapacity"}
...
def test_positive_resource_limit_schemas_share_exclusive_minimum_constraints():
    ...
    for schema_name, field_name in optional_positive_fields:
        field = schemas[schema_name]["properties"][field_name]
        assert field["anyOf"] == [{"type": "integer", "exclusiveMinimum": 0.0}, {"type": "null"}]
