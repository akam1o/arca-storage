...
class TestFormatXfs:
    ...
    def test_format_rejects_existing_non_xfs_filesystem(self, mock_subprocess, mock_path_exists):
        ...
        mock_path_exists.return_value = True
        mock_subprocess.return_value = MagicMock(returncode=0, stdout='TYPE="ext4" /dev/vg_pool_01/vol1 secret-token')
    ...
    def test_format_fails(self, mock_subprocess, mock_path_exists):
        ...
        mock_subprocess.side_effect = [
            MagicMock(returncode=1),  # blkid (not formatted)
            MagicMock(returncode=1, stderr="secret-token /dev/vg_pool_01/vol1"),  # mkfs.xfs fails
        ]
...
class TestMountXfs:
    ...
    def test_mount_fails(self, mock_makedirs, mock_subprocess):
        ...
        mock_subprocess.side_effect = [
            MagicMock(returncode=1),  # mountpoint (not mounted)
            MagicMock(returncode=1, stderr="secret-token /exports/tenant_a/vol1"),  # mount fails
        ]

        with pytest.raises(RuntimeError, match="Failed to mount XFS") as exc_info:
            mount_xfs("/dev/vg_pool_01/vol1", "/exports/tenant_a/vol1")

        _assert_redacted(exc_info.value, "secret-token", "/exports/tenant_a/vol1", "/dev/vg_pool_01/vol1")
...
class TestUmountXfs:
    ...
    def test_umount_fails(self, mock_subprocess):
        ...
        mock_subprocess.side_effect = [
            MagicMock(returncode=0),  # mountpoint (mounted)
            MagicMock(returncode=1, stderr="secret-token /exports/tenant_a/vol1"),  # umount fails
        ]
...
class TestGrowXfs:
    ...
    def test_grow_fails(self, mock_subprocess):
        ...
        mock_subprocess.side_effect = [
            MagicMock(returncode=0),  # mountpoint (mounted)
            MagicMock(returncode=1, stderr="secret-token /exports/tenant_a/vol1"),  # xfs_growfs fails
        ]
